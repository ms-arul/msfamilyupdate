...
export default function Analytics() {
  const { transactions = [], budgetLimit = 0, savingsGoal = { name: 'Family Goal', target: 100000 } } =
    useFinance();
...
const exportData = useCallback(() => {
  const headers = 'Date,Type,Category,Amount,Member,Notes\n';
  const rows = filteredTransactions
    .map(
      (t) =>
        `${t.date},${t.type},${t.category},${t.amount},${t.memberName},${t.notes || ''}`
    )
    .join('\n');
  ...
  a.href = url;
  a.download = `ms_family_analytics_${timeFilter}.csv`;
  a.click();
  URL.revokeObjectURL(url);
}, [filteredTransactions, timeFilter]);

return (
  <motion.div variants={container} initial="hidden" animate="show" className="space-y-4 md:space-y-6 pb-20 md:pb-6 max-w-7xl mx-auto">
    {/* Header & Controls */}
    <motion.div
      variants={item}
    ...
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] md:text-xs text-secondary-400 uppercase tracking-widest font-bold mt-1">
              Cross-referencing financial matrix
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto relative z-10 mt-2 md:mt-0">
        ...
        <div className="flex p-1 bg-white border border-slate-900/10 rounded-xl">
          {[
            { id: 'all', icon: Clock, label: 'All Time' },
            ...
            <button
              ...
              className={`flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-4 py-2 rounded-lg text-xs font-bold transition-all duration-300 ${timeFilter === f.id
        ...
        <button
          onClick={exportData}
          className="btn-glass justify-center uppercase tracking-widest text-[10px] font-bold"
    ...
    <motion.div variants={item} className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
      <KpiCard label="Gross Inflow" value={totalIncome} color="text-success-400" bg="bg-success-500/10" />
      <KpiCard label="Capital Burn" value={totalExpense} color="text-accent-400" bg="bg-accent-500/10" />
      <KpiCard
        label="Net Retained"
        value={netBalance}
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        ...
        <div className="flex-1 min-h-0">
          {trendData.length > 0 ? (
            ...
            <div className="h-full flex items-center justify-center text-slate-600 text-sm font-medium">
              No liquidity data for selected period.
            </div>
          )}
        </div>
      </motion.div>
      ...
      <div className="space-y-4 md:space-y-6 flex flex-col">
        ...
        <motion.div
          variants={item}
          className="glass-panel p-4 md:p-6 flex-1 min-h-[280px] flex flex-col"
        >
          ...
          <div className="flex-1 relative min-h-0">
            {categoryData.length > 0 ? (
              ...
              <div className="h-full flex items-center justify-center text-slate-600 text-sm font-medium">
                Initialize allocations to view.
              </div>
            )}
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="space-y-3">
            {memberData.length > 0 ? (
              ...
              <span className="text-slate-600 text-sm font-medium">No entities active.</span>
            )}
          </div>
        </motion.div>
      </div>
    ...
    <motion.div variants={item} className="glass-panel relative overflow-hidden">
      ...
      <div className="p-4 md:p-6 border-b border-slate-900/5 flex items-center gap-3 bg-white/[0.01]">
        ...
        <div>
          <h3 className="text-sm md:text-base font-extrabold text-slate-900">Synaptic Insights</h3>
