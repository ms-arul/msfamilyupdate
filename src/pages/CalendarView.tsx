    ...
    return (
      <motion.div
        ...
        className={`min-h-[72px] sm:min-h-[80px] p-2 rounded-xl cursor-pointer transition-all duration-300 border relative ${!isCurrentMonth
          ? 'opacity-30 border-transparent pointer-events-none'
          : isSelected
        ...
        whileHover={isCurrentMonth ? { scale: 1.02 } : undefined}
...
// Animation variants
// ============================================================================
const container = staggerContainer(0.03, 0.1);
...
export default function CalendarView() {
  ...
  const calendarData = useMemo(() => {
    ...
    const txMap = new Map<string, { income: number; expense: number; count: number }>();
            ...
            <div className="grid grid-cols-2 gap-2 mb-4">
              <div className="p-3 rounded-xl bg-success-500/10 border border-success-500/20 text-center">
                <div className="flex items-center justify-center gap-1 mb-1">
                  ...
                  <p className="text-[10px] text-slate-600 uppercase font-bold">{t('Income')}</p>
                </div>
              ...
              <div className="p-3 rounded-xl bg-accent-500/10 border border-accent-500/20 text-center">
                <div className="flex items-center justify-center gap-1 mb-1">
                  ...
                  <p className="text-[10px] text-slate-600 uppercase font-bold">{t('Expense')}</p>
                </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-2.5 pr-1 custom-scrollbar">
            {selectedTx.length === 0 ? (
              <div className="text-center py-10 flex flex-col items-center">
                ...
                <p className="text-slate-600 text-sm font-medium">{t('No records on this date')}</p>
                <p className="text-xs text-slate-500 mt-1">{t('Add a transaction to see it here')}</p>
              </div>
            ) : (
              selectedTx.map((tx) => (
                <motion.div
                ...
                >
                  ...
                  <p className="text-sm font-semibold">{t(tx.memberName)}</p>
                  {tx.notes && (
