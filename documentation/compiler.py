...
OUTPUT_HTML = os.path.join(BASE_DIR, 'combined.html')
OUTPUT_PDF = os.path.join(BASE_DIR, 'MS_Family_Architecture_Documentation.pdf')
...
def check_and_install_mermaid():
    """
    Checks if Mermaid CLI is installed locally. If not, installs it.
    """
    node_modules_cli = os.path.join(PROJECT_ROOT, "node_modules", "@mermaid-js", "mermaid-cli")
    
    # If node_modules folder for mermaid-cli exists, we are good
    if os.path.exists(node_modules_cli):
        print("[Mermaid CLI] Found local installation in node_modules.")
        return True
        
    print("[Mermaid CLI] Local installation NOT found. Attempting automatic installation...")
    try:
        # Run npm install -D @mermaid-js/mermaid-cli
        print("  Executing: npm install -D @mermaid-js/mermaid-cli")
        ...
        if result.returncode == 0:
            ...
            print(f"[Error] Failed to install Mermaid CLI: {result.stderr}")
            return False
    except Exception as e:
        print(f"[Error] Exception during Mermaid CLI check/install: {e}")
        return False

def compile_mermaid_offline(mermaid_code, code_hash):
    """
    Compiles Mermaid diagram locally to SVG using local mmdc CLI.
    """
...
def compile_documentation():
    print("=" * 60)
    print("MS Family Technical Documentation Compiler (Enhanced Enterprise Edition)")
    print("=" * 60)
    
    # 1. Check and install Mermaid CLI locally
    check_and_install_mermaid()
    ...
    print(f"Found {len(src_files)} source documents in src/ directory.")
...
<head>
    <meta charset="utf-8">
    <title>MS Family — Technical Design &amp; Architecture Documentation</title>
    <style>
        {styles_content}
    </style>
</head>
<body>
    ...
    <div class="watermark-container">
        <div class="watermark-text">MS Family Technical Documentation</div>
    </div>
...
"""
    ...
    # 5. Run Auto-Numbering & Cross-Referencing Engine
    ...
    # We find all matches of table comments to number them
    temp_html = full_html
    matches = list(table_caption_pattern.finditer(temp_html))
    for match in matches:
        table_title = match.group(1)
        table_count += 1
        
        # We need to find the table element that follows and inject a unique ID if it doesn't have one
        # For simplicity, let's auto-generate a table ID based on title
    ...
    # Replaces <!-- table: My Title --> <table> with caption container + table
    table_index = 0
    ...
    # Custom headers/footers templates (Premium style, page counter, confidential label)
    header_html = """
    <div style="font-size: 7.5pt; font-family: 'Inter', sans-serif; color: #94a3b8; width: 100%; display: flex; justify-content: space-between; padding: 0 24px; border-bottom: 1px solid #e2e8f0; margin-bottom: 10px;">
        <span style="font-weight: 700; color: #0b1530;">MS FAMILY SYSTEM ARCHITECTURE</span>
        <span style="text-transform: uppercase; letter-spacing: 0.5px;">Engineering Design &amp; Specifications Handbook</span>
    </div>
    """
